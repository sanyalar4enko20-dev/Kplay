...
@dp.message(lambda m: m.text and re.fullmatch(r"\d+\s(орёл|орел|решка)", m.text.lower()))
async def coin_flip(msg: types.Message):
    ...
    if choice == result:
        win = bet * 2
        add_balance(uid, win)
        await msg.reply(f"🎉 Победа! Выпало: {result}\n+{win} {CURRENCY}\n💰 Баланс: {get_balance(uid)} {CURRENCY}")
    else:
        await msg.reply(f"💥 Проигрыш! Выпало: {result}\n💰 Баланс: {get_balance(uid)} {CURRENCY}")
...
if __name__ == "__main__":
 asyncio.run(main())USERS_FILE = "users.txt"
...
async def bal_top(call: types.CallbackQuery):
    top = sorted(
        [(u, b) for u, b in balances.items() if u != OWNER_ID],
        key=lambda x: x[1],
        reverse=True
    )[:10]
    text = "🏆 Топ:\n" + "\n".join(f"{i+1}. {u} — {b}" for i, (u, b) in enumerate(top))
    await call.message.edit_text(text or "Пусто", reply_markup=back_kb())
